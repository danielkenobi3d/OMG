import os
from typing import assert_type


asset_list = ['Intern','Senior','WorkerA','MachineB', 'ChairOG', 'MachineA']
asset_types = ['Main Characters', 'Props Lobby', 'Props Office', 'Props']
project_path = 'Z:/__07-CLASSE ANIMATION M2_24-25/OMG/04-ASSETS'
asset_path = 'OMG_A_{}'
model_path = 'OMG_A_{}_Mod'
rig_path = 'OMG_A_{}_Rig'
data_path = f'{os.getlogin()}/data'
publish_folder = '_published'
modules_path = 'OMG.rigBuilds.assets'
default_module = 'default_character'
default_facial_definition = 'facial_definition'
default_geo_definition = 'geo_definition'


