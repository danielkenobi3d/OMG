...
class RigByped(rigBase.RigBase):
    ...
    def build(self):
        ...
        self.l_arm.set_parent(pm.ls('shoulder_bind_JNT')[0], create_hierarchy_joints=True, output_joint_rig = self.l_rig_output)

        self.r_arm.create_point_base(*[each.format('R') for each in self.arm_root])
        self.r_arm.set_parent(pm.ls('shoulder_bind_JNT')[0], create_hierarchy_joints=True, output_joint_rig = self.r_rig_output)
